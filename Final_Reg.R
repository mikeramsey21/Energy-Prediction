...
# ALL MODELS IN THIS R SCRIPT ONLY USE TEMPERATURE AND REGION AS A PREDICTOR. 
# POPULATION IS NOT USED AS A PREDICTOR
...
poolm <- lm(Energy~Temp+as.factor(Region), data = data_long)
...
# Fit a pooled linear model
poolm2 <- lm(Energy~Temp+as.factor(Region), data = data_long)
...
# Fit a pooled linear model
poolm3 <- lm(log(Energy)~Temp+as.factor(Region), data = data_long2)
...
################################################################
stratlm <- lm(Energy~as.factor(Region)+Temp*as.factor(City), data = data_long)
...
stratlm2 <- lm(log(Energy)~as.factor(Region)+Temp*as.factor(City), data = data_long2)
...
################################################################
melm <- lmer(Energy ~ as.factor(Region)+Temp +(1 + Temp | City), 
             data = data_long)
summary(melm)
...
melm2 <- lmer(log(Energy) ~ as.factor(Region)+Temp +(1 + Temp | City), 
              data = data_long2)
summary(melm2)
...
# Fit a poisson model
################################################################
plm <- glm(Energy ~ as.factor(Region)+Temp, 
...
plm2 <- glm(Energy ~ as.factor(Region)+Temp*as.factor(City), 
            family="poisson", data=data_long)
summary(plm2)
...
################################################################  
meplm <- glmer(Energy ~ as.factor(Region)+Temp + (1 + Temp | City), 
               family="poisson", data=data_long)
summary(meplm)
...
################################################################  
zglm <- glmmTMB(Energy ~ as.factor(Region)+Temp + (1 + Temp | City), 
                family="poisson", data=data_long,
                zi=~Temp)
summary(zglm)
